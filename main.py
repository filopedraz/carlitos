        ...
        try:
            ...
            with console.status("[cyan]Thinking...[/cyan]", spinner="dots"):
                try:
                    ...
                    agent_info = ""
                    if (
                        hasattr(agent, "_current_agent_type")
                        and agent._current_agent_type
                    ):
                        if agent._current_agent_type.startswith("coordinator"):
                            agent_info = " [blue](via multi-agent coordination)[/blue]"
                        else:
                            agent_info = (
                                f" [blue](via {agent._current_agent_type} agent)[/blue]"
                            )

                    console.print(
                        f"[bold green]Carlitos:{agent_info}[/bold green] {response}"
                    )
                except Exception as e:
                    logger.error(f"Error processing message: {e}", exc_info=True)
                    console.print(f"[bold red]Error:[/bold red] {str(e)}")
                    console.print(
                        "[yellow]Please try again with a different query.[/yellow]"
                    )
            ...
            console.print(
                "[yellow]Chat loop will continue. You can still enter messages.[/yellow]"
            )
...
def display_debug_info(agent):
    ...
    try:
        if isinstance(
            agent._last_tool_results, str
        ) and agent._last_tool_results.startswith("{"):
            ...
            agent._last_tool_results, str
        ) and agent._last_tool_results.startswith("EXECUTION SUMMARY"):
            # Split into sections for better readability
            parts = agent._last_tool_results.split("\n\nRESULTS:\n")
            if len(parts) == 2:
                console.print(
                    Panel(parts[0], title="Execution Summary", border_style="yellow")
                )
                console.print(
                    Panel(
                        parts[1][:1000] + "..." if len(parts[1]) > 1000 else parts[1],
                        title="Results",
                        border_style="green",
                    )
                )
    ...
    if hasattr(agent, "_current_agent_type") and agent._current_agent_type:
        if agent._current_agent_type in agent.sub_agents:
            ...
            if (
                hasattr(sub_agent, "_last_tool_results")
                and sub_agent._last_tool_results
            ):
                console.print(
                    f"[bold magenta]Debug - {agent._current_agent_type} Sub-Agent Tool Results:[/bold magenta]"
                )
                try:
                    if isinstance(
                        sub_agent._last_tool_results, str
                    ) and sub_agent._last_tool_results.startswith("{"):
    ...
    with console.status("[cyan]Discovering tools...[/cyan]", spinner="dots"):
        try:
            ...
            if not agent.sub_agents:
                ...
                console.print(
                    f"[bold cyan]Initialized {len(agent.sub_agents)} specialized sub-agents[/bold cyan]"
                )

            # Defer tool discovery until necessary
            console.print(
                "[bold cyan]Configured for on-demand tool discovery[/bold cyan]"
            )
...
def run_chat(debug=DEBUG):
    ...
    try:
        ...
        console.print(
            "[bold cyan]Using MegaAgent to route requests to specialized sub-agents[/bold cyan]"
        )

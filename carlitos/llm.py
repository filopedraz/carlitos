...
class LLMCoreAgent:
    ...
    def __init__(self, llm_config: Dict[str, Any]):
        ...
        self.client = Agent(
            model_name, system_prompt=CARLITOS_SYSTEM_PROMPT, instrument=True
        )
    ...
    async def analyze_task(
        ...
        detailed_server_info: Dict[str, str] = None,
        ...
        prompt = self._get_task_analysis_prompt(
            query, available_tools, detailed_server_info, chat_history_formatted
        )
        ...
        try:
            ...
            return (
                "I couldn't properly analyze the task. Let me try to help directly.",
                [],
            )
    ...
    async def synthesize_results(
        ...
        logger.debug(
            f"Sending synthesis prompt to PydanticAI with {len(tool_results)} chars of tool results"
        )
        ...
        for tool in available_tools:
            ...
            if hasattr(tool, "inputSchema") and tool.inputSchema:
                ...
                try:
                        ...
                        for param_name, param_info in properties.items():
                            ...
                            if param_type == "object" and "properties" in param_info:
                                ...
                                for (
                                    ...
                                    params.append(
                                        f"      • {nested_name} (Type: {nested_type}{nested_required_str}): {nested_desc}"
                                    )
                            else:
                                param_str = f"    - {param_name} (Type: {param_type}"
                                if is_required:
                    ...
                    logger.error(
                        f"Error formatting tool parameters for {tool.name}: {e}"
                    )

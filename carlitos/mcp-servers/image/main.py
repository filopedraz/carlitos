...
class OpenAIImageGenServer:
    ...
    def generate_image_4o(
        ...
        logger.info(
            f"generate_image_4o called with prompt: '{prompt}', size: {size}, n: {n}, transparent: {transparent_background}"
        )
        try:
            ...
            result_data = {"success": True, "data": image_url, "model": "gpt-image-1"}
...
async def generate_image(
    ...
    """
    logger.info(
        f"Tool 'generate_image' called with prompt: '{prompt}', size: {size}, n: {n}, transparent: {transparent_background}"
    )
    try:

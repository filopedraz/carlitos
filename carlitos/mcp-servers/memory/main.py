...
class MemoryManager:
    def __init__(self):
        ...
        logger.info(
            f"Initializing MemoryManager with api_key: {'[SET]' if api_key else '[NOT SET]'}"
        )
    ...
    def add_memory(
        ...
        """
        logger.info(
            f"MemoryManager.add_memory called with {len(messages)} messages for user_id: '{user_id}'"
        )
        try:
            ...
            logger.info(
                f"MemoryManager.add_memory returning error response: {response_data}"
            )
            return response_data
    ...
    def search_memory(
        ...
        """
        logger.info(
            f"MemoryManager.search_memory called with query: '{query}', user_id: '{user_id}', limit: {limit}"
        )

        # Check if API key is set
        if not hasattr(self.client, "api_key") or not self.client.api_key:
            logger.error("API key not set for mem0 client")
            return {"success": False, "error": "API key not set for memory service"}

        try:
            # Search mem0 for relevant memories
            mem0_results = self.client.search(query, user_id=user_id, limit=limit)
            logger.info(
                f"mem0 client returned: {type(mem0_results).__name__}, content: {mem0_results}"
            )
            ...
            response_data = {"success": True, "results": mem0_results}
            ...
            response_data = {"success": False, "error": str(e)}
            logger.info(
                f"MemoryManager.search_memory returning error response: {response_data}"
            )
            return response_data

    def add_conversation(
        self, user_message: str, assistant_message: str, user_id: str = "default"
    ) -> Dict[str, Any]:
        """
...
async def add_memory(
    messages: List[Dict[str, str]], user_id: str = "default"
) -> Dict[str, Any]:
    """
    Add messages to memory
    ...
    """
    logger.info(
        f"Tool 'add_memory' called with {len(messages)} messages for user_id: '{user_id}'"
    )
    ...
    clean_result = {"success": result.get("success", False)}
...
async def search_memory(
    query: str, user_id: str = "default", limit: int = 5
) -> Dict[str, Any]:
    """
    Search memory for relevant information
    ...
    """
    logger.info(
        f"Tool 'search_memory' called with query: '{query}', user_id: '{user_id}', limit: {limit}"
    )
    try:
        logger.debug("About to call memory_manager.search_memory")
        result = memory_manager.search_memory(query, user_id, limit)
        logger.debug(
            f"memory_manager.search_memory returned, type: {type(result).__name__}"
        )
        ...
        if not result.get("success", False):
            logger.warning(
                f"search_memory error: {result.get('error', 'Unknown error')}"
            )
        ...
        clean_result = {
            ...
            "results": (
                result.get("results", []) if result.get("success", False) else []
            ),
        }
...
async def add_conversation(
    user_message: str, assistant_message: str, user_id: str = "default"
) -> Dict[str, Any]:
    """
    Add a conversation exchange to memory

...
from carlitos.prompt import ROUTING_PROMPT, CLARIFICATION_QUESTION_PROMPT, ROUTING_SYSTEM_PROMPT, CLARIFICATION_SYSTEM_PROMPT
...
class RoutingAgent:
    ...
    def __init__(self, routing_config: Dict[str, Any], server_descriptions: Dict[str, str]):
        """
    ...
    async def identify_relevant_agents(self, message: str, integration_types: List[str], formatted_history: str) -> Dict[str, List[str]]:
        """
        ...
        try:
            ...
            if len(valid_integrations) < len(integrations):
                logger.warning(f"Some selected integrations are not available: {set(integrations) - set(valid_integrations)}")
    ...
    async def ask_clarification_question(self, message: str, formatted_history: str, available_integrations: str) -> str:
        """
